...
def write_WR(agent):
  ...
  for i in xrange(1,len(arr_WR)-2):
    ...
    for s in Ser_cmp_proc: # Ser_cmp_proc
      if s.st_proc != '':
        if t(s.st_proc) >= arr_WR[i].week and t(s.st_proc) < arr_WR[i+1].week and s.proc == agent:
      ...
      if s.st_ia1 != '':
        if t(s.st_ia1) >= arr_WR[i].week and t(s.st_ia1) < arr_WR[i+1].week and s.ia1 == agent:
    ...
    for s in Ind_cmp: # Ind_cmp
      ...
      if s.st_ia2 != '':
        if t(s.st_ia2) >= arr_WR[i].week and t(s.st_ia2) < arr_WR[i+1].week and s.ia2 == agent:

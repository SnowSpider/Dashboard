...
arr_agent = []
collectAgents('Source/Agents_ATX.csv', arr_agent)
...
date_start = datetime(2018, 10, 29, h_start_shift)
date_end = datetime(2019, 12, 31, h_end_shift)
...
for i in xrange(1,len(arr_WR)-2):
  ...
  for s in Ser_cmp_proc: # Ser_cmp_proc
    if s.st_proc != '':
      if t(s.st_proc) >= arr_WR[i].week and t(s.st_proc) < arr_WR[i].week+timedelta(hours=9) and s.proc in arr_agent:
    ...
    if s.st_pa1 != '':
      if t(s.st_pa1) >= arr_WR[i].week and t(s.st_pa1) < arr_WR[i].week+timedelta(hours=9) and s.pa1 in arr_agent:
        arr_WR[i].pa1_Ser_cmp += 1
    if s.st_ing != '':
      if t(s.st_ing) >= arr_WR[i].week and t(s.st_ing) < arr_WR[i].week+timedelta(hours=9) and s.ing in arr_agent:
        arr_WR[i].ing_Ser_cmp += 1
    if s.st_ia1 != '':
      if t(s.st_ia1) >= arr_WR[i].week and t(s.st_ia1) < arr_WR[i].week+timedelta(hours=9) and s.ia1 in arr_agent:
  ...
  for s in Rng_cmp_proc: # Rng_cmp_proc
    ...
    if s.st_pa2 != '':
      if t(s.st_pa2) >= arr_WR[i].week and t(s.st_pa2) < arr_WR[i].week+timedelta(hours=9) and s.pa2 in arr_agent:
  ...
  for s in Ind_cmp: # Ind_cmp
    ...
    if s.st_ia2 != '':
      if t(s.st_ia2) >= arr_WR[i].week and t(s.st_ia2) < arr_WR[i].week+timedelta(hours=9) and s.ia2 in arr_agent:
  ...
  for s in Parser_ing: # Parser_ing
    if s.st_ing != '':
      if t2(s.st_ing) >= arr_WR[i].week and t2(s.st_ing) < arr_WR[i].week+timedelta(hours=9) and s.ing + '@google.com' in arr_agent:
        arr_WR[i].ing_Parser_ing += 1
    if s.st_ia1 != '':
      if t2(s.st_ia1) >= arr_WR[i].week and t2(s.st_ia1) < arr_WR[i].week+timedelta(hours=9) and s.ia1 + '@google.com' in arr_agent:
        arr_WR[i].ia1_Parser_ing += 1
    if s.st_ia2 != '':
      if t2(s.st_ia2) >= arr_WR[i].week and t2(s.st_ia2) < arr_WR[i].week+timedelta(hours=9) and s.ia2 + '@google.com' in arr_agent:
  ...
  with open('Source/Parser_cmp.csv') as csvfile:
    ...
    next(csvreader)
    for row in csvreader:
      if t(row[col_to_num('A')]) >= arr_WR[i].week and t(row[col_to_num('A')]) < arr_WR[i].week+timedelta(hours=9) and row[col_to_num('G')] in arr_agent:
        if row[col_to_num('E')]!='': 
...
csvfile = open('Output/DR_ATX_S2.csv','w')
d = ','
